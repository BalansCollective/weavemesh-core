//! Standalone demonstration of the financial framework
//! This file can be compiled and run independently to show the framework works

use std::collections::HashMap;
use chrono::{DateTime, Utc};
use serde::{Deserialize, Serialize};

// Standalone error type for this demo
#[derive(Debug)]
pub enum DemoError {
    Financial(String),
}
...
impl std::fmt::Display for DemoError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DemoError::Financial(msg) => write!(f, "Financial error: {}", msg),
        }
    }
}
...
// Copy the financial framework types for standalone demo
...
impl FinancialTracker {
    ...
    pub fn record_cost(&mut self, record: CostRecord) -> Result<(), DemoError> {
    ...
    ) -> Result<ApprovalResult, DemoError> {
    ...
    pub fn get_spending_for_period(&self, period: SpendingPeriod) -> Result<u64, DemoError> {
        let now = Utc::now();
        let cutoff = match period {
            ...
            SpendingPeriod::Session => now - chrono::Duration::hours(1),
    ...
    pub fn get_spending_summary(&self, period: SpendingPeriod) -> Result<SpendingSummary, DemoError> {
        let now = Utc::now();
...
impl FinancialManager {
    ...
    ) -> Result<(u64, ApprovalResult), DemoError> {
    ...
    pub fn get_summary(&self, period: SpendingPeriod) -> Result<SpendingSummary, DemoError> {
...
fn main() -> Result<(), DemoError> {
    println!("🏦 WeaveMesh Financial Framework Standalone Demo");
    println!("===============================================\n");
